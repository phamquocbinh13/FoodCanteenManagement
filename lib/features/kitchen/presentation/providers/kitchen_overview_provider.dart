import 'package:flutter_riverpod/flutter_riverpod.dart';

import '../../../../app/config/restaurant_context.dart';
import '../../../../app/di/injection.dart';
import '../../../../application/kitchen/kitchen_overview_view.dart';
import '../../../../application/usecases/kitchen/get_kitchen_overview_use_case.dart';
import '../../../../core/result/result.dart';

/// Lazy kitchen overview — loads when Overview tab is first watched.
final kitchenOverviewProvider =
    FutureProvider.autoDispose<KitchenOverviewView>((ref) async {
  final result = await sl<GetKitchenOverviewUseCase>()(
    GetKitchenOverviewParams(
      restaurantId: sl<RestaurantContext>().restaurantId,
    ),
  );
  return switch (result) {
    Success(:final value) => value,
    Err(:final failure) => throw Exception(failure.message),
  };
});
